...
`return` from inside a block exits the enclosing function as usual; `break` and
`continue`, however, belong at statement position in a loop body rather than
buried inside a block's sub-expressions (see KNOWN_LIMITATIONS).
...
- Restore the value-stack base across `break`/`continue` so a non-local exit
  from inside a sub-expression is balanced (today they belong at statement
  position; see KNOWN_LIMITATIONS)
- Auto-growing indexed assignment; assignment to `rec.field[i]`

Done since the last cut: block expressions — `( s1; s2; expr )` as a scoped
